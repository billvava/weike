    ...
    class MembercenterController extends Aclfilter{
        ...
        */
        public function actionWangwangdel()
        {
            ...
            会员管理-退出商保审核
        */
        public function actionExitprotectlist()
        {
            parent::acl();
			/*
            if(isset($_POST['keyword']))
            {
                ...
                $proreg = Blindwangwang::model()->findAll($criteria);
                //分页结束
                $this->renderPartial('exitprotectlist',array(
                    'proInfo' => $proreg,
                    'pages' => $pages
                ));
            }
			...
			*/
            {
               ...
               // $criteria->condition="status=0";
                ...
                //分页开始
                $total = Exitprotectplanrecord::model()->count($criteria);
                $pages = new CPagination($total);
                $pages->pageSize=12;//分页大小
                $pages->applyLimit($criteria);
                
                $proreg = Exitprotectplanrecord::model()->findAll($criteria);
                //分页结束
                $this->renderPartial('exitprotectlist',array(
                    'proInfo' => $proreg,
                    'pages' => $pages
		  ...
		  /*
            ***手动通过退出商保
        */
        public function actionLetExitprotectPass()
        {
            if(isset($_GET['authid']) && $_GET['authid']!="")
            {
				$uid = $_GET['authid'];
                ...
                //$userinfo=User::model()->findByAttributes(array("Username"=>$_POST['keyword']));
                $Exitprotectplaninfo=Exitprotectplanrecord::model()->findByAttributes(array("uid"=>$uid));

                $Exitprotectplaninfo->exitTime=time();
                $Exitprotectplaninfo->status=1;//审核通过
                $Exitprotectplaninfo->save();

				//金额退到账户余额
				$userinfo = User::model()->findByPk($uid);
				$userinfo->JoinProtectPlan = 0;
				$userinfo->JoinProtectPlanTime = null;
				$userinfo->Money = $userinfo->Money + $userinfo->JoinProtectPlanMoney;
				$userinfo->JoinProtectPlanMoney = 0;
				$userinfo->save();
		  ...
		  /*
            会员管理-删除退出商保
        */
        public function actionExitprotectdel()
        {
            parent::acl();
            if(isset($_GET['id']))
            {
