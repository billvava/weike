...
class SiteController extends Controller
{
    ...
    /*
        ***迅达维支付api，即将用户充值到支付宝之后将数据存入到kcborder表，方便用户通过提交交易号完成平台充值
    */
    public function actionPayNotify()
    {
		
         if(isset($_POST['tradeNo']) && $_POST['tradeNo']!="" && isset($_POST['money']) && $_POST['money']!="")
         {
            //检查订单号是否已存在
            $checkExist=Kcborder::model()->findByAttributes(array("tno"=>$_POST["tradeNo"]));
            if($checkExist)//订单已存在
            {
                //做错误存储信息，以便日后查问题帐户
                exit('已经存在');   
			...
			else//订单不存在
            {
                ...
                //密钥检测
				$key = "Abc123456";	//通讯秘钥，必须与软件的通讯秘钥字符串一致，请修改，勿用此默认值
			 
				$sig = $_POST['sig'];//签名
                $tradeNo=$_POST["tradeNo"];//支付宝订单号 
                $desc=$_POST["desc"];//交易名称
                $time=$_POST["time"];//交易时间
				$username=$_POST["username"];//对方名称
				$userid=$_POST["userid"];//用户id
			    $money=$_POST["money"];//金额
				$status=$_POST["status"];//交易状态
 				if(strtoupper(md5("$tradeNo|$desc|$time|$username|$userid|$money|$status|$key")) == $sig)
				{
					$kcbOrder=new Kcborder();
                    $kcbOrder->tno=$tradeNo;//支付宝订单号 
                    $kcbOrder->payno=$desc;//编号备注 一般是用户名
                    $kcbOrder->money=$money;//金额
                    $kcbOrder->addtime=time();//加入时间，软件通过接口将充值充值的数据添加到kcborder表的时间
    ...
    */
        ...
        if(isset($_POST['taskerWangwang']) && isset($_POST['taskid']) && Yii::app()->user->getId())
        {
            ...
            if($taskInfo->status!=6 && $taskInfo->status==0)//任务处于可接状态
            {
                ...
                if($taskInfo->cbxIsFMaxMCount){//如果任务对接手接任务数量有限制则进行过滤
                    ...
                    if($taskInfo->fmaxmc==2 && count($limitTaskNumOneDay)>1)//一天接不超过2单
                    {
            ...
            if($taskInfo->save())
            {
                ...
                if($userinfo->save() && $publishinfo->save())
                {
                    ...
                    if($userinfo->IdNumber && $userAddMoney=User::model()->findByPk($userinfo->IdNumber))
                    {
                        ...
                        if($recordCheck==false)//如果给过上线的奖励记录，则进行相关判断操作
                        {
                            ...
                            if(count($TakeTaskWcCount)>9 || count($PublishTaskWcCount)>9)//接手的任务或者发布的任务完成10个即奖励
                            {
                                $spreadPrice=System::model()->findByAttributes(array("varname"=>"spreadPrice"));
                                $checkrecord=Recordlist::model()->findByAttributes(array("userid"=>$uid,"spreadid"=>$userinfo->id));
                                if($checkrecord==false)//如果没有奖励过，则进行奖励
                                {
                                    //推广奖励返现流水
                                    $recordSpreak=new Recordlist();
                                    $recordSpreak->userid=$uid;//上家id
                                    $recordSpreak->catalog=12;//推广返现类型
                                    $recordSpreak->number=$spreadPrice->value;//返现金额
                                    $recordSpreak->time=time();//操作时间
                                    $recordSpreak->spreadid=$userinfo->id;//下线即接任务者的id
                                    $recordSpreak->save();//保存米粒流水
                                    
                                    //改变上家余额
                                    $userAddMoney->Money=$userAddMoney->Money+$spreadPrice->value;//达到要求奖励18元
                                    $userAddMoney->SpreadMoney=(int)$userAddMoney->SpreadMoney+$spreadPrice->value;//累加推广获得的返现金额
                                    $userAddMoney->save();
                                }
                            }
                        }
                    ...
                    if($myinfo->IdNumber && $userAddMoney=User::model()->findByPk($myinfo->IdNumber))//如果我有介绍人
                    {
                        $spreadid=Yii::app()->user->getId();//下线id
                        $uid=$myinfo->IdNumber;//上线id
                        ...
                        if($recordCheck==false)//如果给过上线的奖励记录，则进行相关判断操作
                        {
                            ...
                            if(count($TakeTaskWcCount)>9 || count($PublishTaskWcCount)>9)//接手的任务或者发布的任务完成10个即奖励
                            {
                                ...
                                if($checkrecord==false)//如果没有奖励过，则进行奖励
                                {
                                    ...
                                    $recordSpreak->number=$spreadPrice->value;//返现金额
                                    $recordSpreak->time=time();//操作时间
                                    $recordSpreak->spreadid=$myinfo->id;//下线即接任务者的id
                                    $recordSpreak->save();//保存米粒流水
    ...
    public function actionAddMinLi()
    {
        if(isset($_POST['addMinLiNum']) && isset($_POST['id']) && Yii::app()->user->getId())
        {
            ...
            {
                //检查米粒数量是否充足
                if($_POST['addMinLiNum']>$userinfo->MinLi)//米粒不足
                {
                    echo "MINLINOTENOUGH";
                }else//米粒充足
                {
    ...
    */
    ...
    public function actionSms()
    {
        if(isset($_POST['phone']) && isset($_POST['phoneCode']))
        {
            ...
            $content=$this->newSms($_POST['phone'],'【淘优客】您的短信验证码为'.$code.'，若非本人操作，请忽略！');
