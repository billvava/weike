...
class UserController extends Controller
{
    ...
    */
    ...
    {
        if(isset($_POST['businessRecord']) && $_POST['businessRecord']!="" && isset($_POST['businessMoney']) && $_POST['businessMoney']!="")
        {
             //根据交易号进行查找
            ...
            if(count($kcbOrder)==1)//交易号存在则进行检查
            {
                if($kcbOrder->status==0 && $kcbOrder->uid==0)//如果存在符合条件
                {
					...
					if($_POST['businessMoney'] != $nowNumber)
					{
					   echo "ERRORMONEY";
                        Yii::app()->end();  
					}
    ...
    */
    public function actionuserPayDetailTask()
    {
        //查询用户充值记录
        $criteria = new CDbCriteria;
        $criteria->condition='userid='.Yii::app()->user->getId().' and catalog in(3,4,5,6)';
        $criteria->order ="id desc";
